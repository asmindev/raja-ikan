        ...
        try {
            ...
            if (stats.size > maxSize) {
                ...
                const logFiles = files
                    .filter(
                        (f) =>
                            f.startsWith("wa-gateway.") && f.endsWith(".log"),
                    )
                    .sort()
                    .reverse();
...
class WhatsAppService {
    ...
    async onConnect() {
        ...
        if (this.socket?.user) {
            ...
            this.logger.info(
                `\uD83D\uDC64 Logged in as: ${this.userInfo.name}`,
            );
            this.connectionUpdateCallback?.({
                connected: true,
                user: this.userInfo,
...
class WebSocketService {
    ...
    emitConnectionStatus(status) {
        this.logger.info(
            `\uD83D\uDD04 Broadcasting connection status: ${status.status}`,
        );
    ...
    emitMessageReceived(data) {
        this.logger.info(
            `\uD83D\uDCE8 Broadcasting incoming message from: ${data.from}`,
        );
...
async function executeGetProducts(args) {
    try {
        ...
        const formattedProducts = products.map(
            (p, index) => `${index + 1}. ${p.name} - ${formatPrice(p.price)}`,
        ).join(`
`);
        return {
            success: true,
...
After extract_order_items returns data, then you can respond in Indonesian.`,
    ...
    async chat(userId, message) {
        try {
            ...
            this.logger.debug(
                `Available functions: ${tools[0]?.functionDeclarations?.map((f) => f.name).join(", ") || "none"}`,
            );
            const contents = [];
            ...
            this.logger.debug(
                `Gemini response - functionCalls: ${response.functionCalls?.length || 0}, text: ${response.text?.substring(0, 100)}`,
            );
            while (
...
class Order {
    ...
    getItemsSummary() {
        return this.items.map((item, idx) => `${idx + 1}. ${item.toString()}`)
            .join(`
`);
    }
    toJSON() {
...
async function sendInteractiveButtons(sock, to, text, buttons, options) {
    const nativeButtons = buttons.map((btn) =>
        proto2.Message.InteractiveMessage.NativeFlowMessage.NativeFlowButton.create(
            {
                name: "quick_reply",
    ...
    extractOrderFromResponse(aiResponse) {
        ...
        if (aiResponse.functionCalls && aiResponse.functionCalls.length > 0) {
            ...
            const orderExtraction = aiResponse.functionCalls.find(
                (fc) => fc.function === "extract_order_items",
            );
            if (orderExtraction) {
                this.logger.info(
...
class ConfirmOrderUseCase {
    ...
    async execute(request) {
        ...
        try {
            ...
            const pendingOrder =
                await this.orderService.getPendingOrder(customerPhone);
            if (!pendingOrder) {
            ...
            const confirmedOrder =
                await this.orderService.confirmOrder(customerPhone);
            if (!confirmedOrder) {
    ...
    async execute(request) {
        ...
        try {
            ...
            const cancelledOrder =
                await this.orderService.cancelOrder(customerPhone);
            const cancelMessage =
...
async function startApp() {
    try {
        ...
        logger8.info(
            `\uD83C\uDF10 Frontend should connect to: http://0.0.0.0:${port}`,
        );
    } catch (error) {
